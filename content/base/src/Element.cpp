                 ...
                 const TimingInput& aTiming, ErrorResult& rv)
{
  ...
  for (uint32_t i = 0; i < keyframes.Length(); i++) {
    JS::Rooted<JSObject*> keyframe(cx, keyframes[i]);
    ...
    for (uint32_t i = 0; i < props.length(); i++){
      ...
      if (!JS_IdToValue(cx, props[i], &cssPropertyName) ||
          !cssPropertyName.isString()) {
        return anim.forget();
      }

      binding_detail::FakeDependentString propertyName;
      ...
      JS::Rooted<jsid> prop(cx, props[i]);
      if (!JS_GetPropertyById(cx, keyframe, prop, &val)) {
        return anim.forget();
      }
