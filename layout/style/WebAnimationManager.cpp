...
{
  nsRefPtr<dom::Animation> anim = new dom::Animation(aElement, aTiming);
...
void
WebAnimationManager::EnsureStyleRuleFor(dom::Animation* aAnim)
{
                                      ...
                                      nsCSSPseudoElements::Type aPseudoType)
{
  ...
  dom::Animation* anim = aElement->GetAnimations()[0];
  if (!anim) {
    return nullptr;
  }
...
WebAnimationManager::CheckNeedsRefresh()
{
  for (uint32_t i = 0; i < mAnimations.Length(); i++) {
    dom::Animation* anim = mAnimations[i];
